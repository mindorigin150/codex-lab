...
fn sqlite_write_failure_should_block(patch: &ThreadMetadataPatch) -> bool {
    ...
    // the existing SQLite value to preserve unspecified fields. User-selected collaboration mode
    // and pin state are SQLite-only, so losing either write must also fail the explicit metadata
    // update.

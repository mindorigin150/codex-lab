...
class InstallCodexLabShTest(unittest.TestCase):
    def test_latest_release_downloads_target_package_and_installs_atomically(
        self,
    ) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            root = Path(temp_dir)
            archive, checksum, metadata = write_release_fixture(root)

            result, requests = run_release_installer(root, archive, checksum, metadata)

            self.assertEqual(result.returncode, 0, result.stderr)
            self.assertEqual(
                requests,
                [
                    "https://api.github.com/repos/mindorigin150/codex-lab/releases/latest",
                    "https://github.com/mindorigin150/codex-lab/releases/download/codex-lab-v1.2.3/codex-package_SHA256SUMS",
                    "https://github.com/mindorigin150/codex-lab/releases/download/codex-lab-v1.2.3/codex-package-x86_64-unknown-linux-musl.tar.gz",
                ],
            )
            release = (
                root / ".local/lib/codex-lab/releases/1.2.3-x86_64-unknown-linux-musl"
            )
            self.assertTrue((release / "codex-package.json").is_file())
            self.assertTrue((release / "bin/codex").is_file())
            self.assertEqual(
                os.readlink(root / ".local/lib/codex-lab/current"),
                "releases/1.2.3-x86_64-unknown-linux-musl",
            )

    def test_release_checksum_mismatch_leaves_current_install_untouched(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            root = Path(temp_dir)
            archive, checksum, metadata = write_release_fixture(root)
            checksum.write_text(
            ...
            result, _requests = run_release_installer(root, archive, checksum, metadata)

            self.assertNotEqual(result.returncode, 0)
            self.assertIn("SHA-256 mismatch", result.stderr)
            self.assertFalse((root / ".local/lib/codex-lab/current").exists())
...
def write_release_fixture(root: Path) -> tuple[Path, Path, str]:
    package = root / "package"
    (package / "bin").mkdir(parents=True)
    (package / "codex-path").mkdir()
    (package / "codex-resources").mkdir()
    (package / "codex-package.json").write_text("{}\n")
    fake_codex = write_fake_codex(package / "bin")
    fake_codex.rename(package / "bin" / "codex")
    write_fake_bwrap(package / "codex-resources" / "bwrap")
    (package / "codex-path" / "rg").write_text("rg\n")
    (package / "codex-path" / "rg").chmod(0o755)
...
def run_release_installer(
    root: Path, archive: Path, checksum: Path, metadata: str
) -> tuple[subprocess.CompletedProcess[str], list[str]]:
    tool_bin = root / "tool-bin"
    tool_bin.mkdir()
    request_log = root / "requests.log"
    fake_curl = tool_bin / "curl"
    ...
    env = {
        **os.environ,
        ...
        "CODEX_LAB_RELEASE": "latest",
        "CODEX_LAB_TARGET": "x86_64-unknown-linux-musl",
        "CODEX_LAB_RUN_DOCTOR": "false",
        "CODEX_LAB_STRIP_BINARY": "false",
        "CODEX_TEST_ARCHIVE": str(archive),
        "CODEX_TEST_CHECKSUM": str(checksum),
        "CODEX_TEST_METADATA": metadata,
        "CODEX_TEST_REQUEST_LOG": str(request_log),
    }
    result = subprocess.run(
        [INSTALL_SCRIPT, "--skip-doctor", "--no-strip"],
        check=False,
        capture_output=True,
        text=True,
        env=env,

...
impl ThreadRequestProcessor {
    ...
    async fn thread_resume_inner(
        ...
        let initial_collaboration_mode = persisted_metadata
            ...
            .and_then(|collaboration_mode| {
                if !has_explicit_model_resume_override {
                    return Some(collaboration_mode);
                }
                config.model.clone().map(|model| {
    ...
    async fn thread_fork_inner(
        ...
        let live_collaboration_mode = match self.thread_manager.get_thread(source_thread_id).await {
        ...
        let history_collaboration_mode =
        ...
        let inherited_collaboration_mode = live_collaboration_mode
            .or(source_thread.collaboration_mode.clone())
            .or(history_collaboration_mode);
        let inherited_reasoning_effort = inherited_collaboration_mode
            .as_ref()
            .map(|mode| mode.settings.reasoning_effort.clone())
            .unwrap_or_else(|| source_thread.reasoning_effort.clone());
        ...
        let has_explicit_model_fork_override =
            has_model_resume_override(request_overrides.as_ref(), &typesafe_overrides);
        let has_explicit_provider_fork_override = typesafe_overrides.model_provider.is_some()
            || request_overrides
                .as_ref()
                .is_some_and(|overrides| overrides.contains_key("model_provider"));
        if let Some(collaboration_mode) = collaboration_mode.as_ref() {
            typesafe_overrides.model = Some(collaboration_mode.settings.model.clone());
            if !has_explicit_provider_fork_override {
                typesafe_overrides.model_provider = Some(source_thread.model_provider.clone());
            }
        } else if !has_explicit_model_fork_override {
            typesafe_overrides.model = inherited_collaboration_mode
                .as_ref()
                .map(|mode| mode.settings.model.clone())
                .or_else(|| source_thread.model.clone());
            typesafe_overrides.model_provider = Some(source_thread.model_provider.clone());
            if let Some(reasoning_effort) = inherited_reasoning_effort.as_ref() {
                request_overrides.get_or_insert_with(HashMap::new).insert(
                    "model_reasoning_effort".to_string(),
                    serde_json::Value::String(reasoning_effort.to_string()),
                );
        ...
        let mut config = self
            ...
            .await
            .map_err(|err| config_load_error(&err))?;
        if !has_explicit_model_fork_override && inherited_reasoning_effort.is_none() {
            config.model_reasoning_effort = None;
        }
        if let Some(collaboration_mode) = collaboration_mode.as_ref() {
            config.model = Some(collaboration_mode.settings.model.clone());
            config.model_reasoning_effort = collaboration_mode.settings.reasoning_effort.clone();
        }
        let initial_collaboration_mode =
            collaboration_mode.or_else(|| match inherited_collaboration_mode {
                Some(collaboration_mode) if has_explicit_model_fork_override => {
                    config.model.clone().map(|model| {
                        collaboration_mode.with_updates(
                            Some(model),
                            Some(config.model_reasoning_effort.clone()),
                ...
                inherited_collaboration_mode => inherited_collaboration_mode,
            });

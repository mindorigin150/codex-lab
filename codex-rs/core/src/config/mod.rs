...
"#;
const DEFAULT_MULTI_AGENT_V2_MODEL_OVERRIDE_USAGE_HINT_TEXT: &str = "Spawned agents inherit the parent model and reasoning effort by default. Set `model` or `reasoning_effort` only when explicitly requested by the user, applicable `AGENTS.md` instructions, or skill instructions; validated overrides are also supported for non-explorer full-history forks. Explorer agents always require fresh context.";
...
fn resolve_multi_agent_v2_config(config_toml: &ConfigToml) -> MultiAgentV2Config {
    ...
    let tool_namespace = tool_namespace.map(|namespace| {

...
impl ThreadManager {
    ...
    pub async fn start_thread_with_options(
        ...
        self.start_thread_with_options_and_collaboration_mode(options, None)
            .await
    }

    pub async fn start_thread_with_options_and_collaboration_mode(
        &self,
        options: StartThreadOptions,
    ...
    pub async fn spawn_subagent(
        ...
        self.start_thread_with_options_and_fork_source(options, Some(forked_from_thread_id), None)
            .await
    }
    ...
    pub async fn resume_thread_with_history(
        ...
        self.resume_thread_with_history_and_collaboration_mode(
            config,
            initial_history,
            auth_manager,
            parent_trace,
    ...
    pub async fn resume_thread_with_history_and_collaboration_mode(
        &self,
        config: Config,
        initial_history: InitialHistory,
        auth_manager: Arc<AuthManager>,
    ...
    where
        S: Into<ForkSnapshot>,
    {
        self.fork_thread_from_history_and_collaboration_mode(
            snapshot,
            config,
            history,
            thread_source,
    ...
    #[allow(clippy::too_many_arguments)]
    pub async fn fork_thread_from_history_and_collaboration_mode<S>(
        &self,
        snapshot: S,
        config: Config,
        history: InitialHistory,
...
impl ThreadManagerState {
    ...
    pub(crate) async fn spawn_thread_with_source(
        ...
        .await?;
        let new_thread = self
            .finalize_thread_spawn(session, io, tracked_session_source, writer_lease)
            .await?;
        if is_resumed_thread {
            new_thread.thread.emit_thread_resume_lifecycle().await;
        }
        if is_primary_resume && multi_agent_version == Some(MultiAgentVersion::V2) {
            self.close_stale_v2_descendants(new_thread.thread_id).await;
        }
        Ok(new_thread)
    }

...
impl UnifiedExecProcessManager {
    ...
    pub(crate) async fn write_stdin(
        ...
        let (locked_process, _interaction_lease) = {
            ...
            let interaction_lease = process.begin_interaction();
            (process, interaction_lease)
        };
    ...
    fn prune_processes_if_needed(store: &mut ProcessStore) -> Option<ProcessEntry> {
        ...
        while let Some(process_id) = Self::process_id_to_prune_from_meta(&meta) {
            ...
            if !candidate_process
                .as_ref()
                .is_some_and(|process| process.has_pending_interaction())
                && let Some(interaction_lock) = candidate_process
                    .as_ref()
